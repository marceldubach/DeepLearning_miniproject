...
parser.add_argument('--cifar',
                    action='store_true', default=False,
                    help = 'Use the CIFAR data_-set and not MNIST (default False)')

parser.add_argument('--data_dir',
                    type = str, default = None,
                    help = 'Where are the PyTorch data_ located (default $PYTORCH_DATA_DIR or \'./data_\')')
...
def load_data(cifar = None, one_hot_labels = False, normalize = False, flatten = True):
    ...
    if args.full:
        ...
        if args.tiny:
            print('** Reduce the data_-set to the tiny setup')
            train_input = train_input.narrow(0, 0, 500)
            train_target = train_target.narrow(0, 0, 500)
            test_input = test_input.narrow(0, 0, 100)
            test_target = test_target.narrow(0, 0, 100)
        else:
            print('** Reduce the data_-set (use --full for the full thing)')
